...
var movil = "movil=3183592508",
    // url = "http://192.168.1.112/?callback=?",
    // url1 = "http://192.168.1.112/",
...
function transitoCaso(){
    ...
    $.getJSON(url, formData, function(json) {
        if(!json.hasOwnProperty('success')){
            if(json.SATISFACTORIO === "True"){
                alert("Servicio en tránsito");
                reloadMenuOption();
            }else{
                alert("No se pudo poner el servicio en tránsito");
                $.mobile.changePage($('#page5'),{transition:'none'});
            }
        }else if(json.success === 'error'){
            alert("error de conexión");
    ...
    $.getJSON(url, formData, function(json) {
        ...
        if(!json.hasOwnProperty('success')){
            if (json.APERTURASATISFACTORIA === "True") {
                alert("Servicio abierto");
                reloadMenuOption();
            }else{
                alert("No se pudo abrir el servicio");
                $.mobile.changePage($('#page5'),{transition:'none'});
            }
        ...
        success: function(json) {
            if(!json.hasOwnProperty('success')){
                ...
                if(json.CIERRESATISFACTORIO === "True"){
                    alert("El servicio ha sido cerrado");
                    reloadMenuOption();
                    $('#page28 content-canvas').html('');
                }else{
                    alert("El servicio no ha sido cerrado");
                }
                $.mobile.loading('hide');
...
function preview(formData){
    ...
    var key = "idReparacion",
        formData3 = methods[4]+"&"+key+"="+sessionStorage.getItem(key),
        dataModules;
    ...
    $.getJSON(url, formData3, function(json) {
        if(!json.hasOwnProperty('success')){
            formData2[1]+=" "+(formData2[2].split("="))[1];
            $page.html('');
            $page.append(' <p><strong>Número de servicio </strong><br>'+json.IDTICKET+'</p>'+
                '<p><strong>Datos del equipo <br>'+
                'Equipo:</strong> '+json.LUNO+' <strong>Serial:</strong> '+json.SERIAL+
                '</strong><br/>'+
                '<strong>Cliente:</strong> '+json.CLIENTE+'</p>'+
                '<p><strong>Tareas</strong><br>'
            );
            for (var j in dataModules){
                $page.append(
                    '<p><strong>Modulo:</strong> '+dataModules[j].nombre+'<br/>'+
                    '<strong>Problema:</strong> '+dataModules[j].problema+'<br/>'+
                    '<strong>Causal:</strong> '+dataModules[j].causal+'<br/>'+
                    '<strong>Cierre</strong>: '+dataModules[j].cierre+'<br/>'+
                    '<strong>Reparación:</strong> '+dataModules[j].reparacion+'<br/>'+
                    '<strong>Comentario:</strong> '+dataModules[j].descripcion+'<br/>'+
                    '<p/>'
                );
            }
            correo= '';
            ...
            $page.append('</p><p><strong>Datos de la persona de contacto</strong> <br>'+
                ...
                '</p>'+
                '<p><strong>Fecha y Hora de Apertura</strong> <br>'+
                json.FECHAAPERTURA+
                '</p>'+
                '<p><strong>Fecha y Hora de cierre</strong> <br>'+
                (formData2[1].split("="))[1]+
                '</p>'
            );
        }else if(json.success === 'error'){
    ...
    //Carga los datos del móduloObtenerReparacionModulosVistaPrevia
    var key2 = "idReparacion",
        formData4 = methods[49]+"&"+key+"="+sessionStorage.getItem(key),
        dataModules2;
    dataModules2 = sessionStorage.getItem('modulo');
    dataModules2 = JSON.parse(dataModules2);
    
    $.getJSON(url, formData4, function(json2) {
        if(!json2.hasOwnProperty('success')){
            formData2[1]+=" "+(formData4[2].split("="))[1];
            $page.html('');
            $page.append(
                '<p><strong>Número de servicio: </strong><br>'+json2.IDTICKET+'</p>'+
                '<p>'+
                    '<strong>Datos del Equipo: </strong><br>'+
                    '<strong>Equipo:</strong> '+json2.LUNO+' <br>'+
                    '<strong>Serial:</strong> '+json2.SERIAL+
                    '<strong>Cliente</strong<br>'+ json2.CLIENTE+
                '</p>'+
                '<p><strong>Tareas</strong><br>'
            );
